...
algorithm: backtracking (dfs) with an optimal filling order
1. Build a list (or a set) for each cell to store the possible answers of the cell
2. Find the cell with least possible answers. Fill the cell with one of the possible answers. Update the possible answers of other cells.
3. Repeat step 2 until solved. Or if there's some cells without possible answers, undo and try the next possible answer.
"""
...
class SudokuSolver(object):
    ...
    def read_board(self, charBoard):
        self.initial_board = [
            [self.char_to_int(char) for char in row] for row in charBoard]
        self.board = [[self.digit_unfilled for char in row]
    ...
    def init_solving_utils(self):
        self.psbl_digits = [[set(range(self.digit_min, self.digit_max + 1))
                             for ci in range(self.col_size)] for ri in range(self.row_size)]
        self.unfilled_cells = []
    ...
    def solve_by_dfs(self):
        ...
        self.unfilled_cells.sort(reverse=True, key=lambda cell: len(
            self.psbl_digits[cell[0]][cell[1]]))
